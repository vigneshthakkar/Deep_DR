...
def restore(sess):
    saver=tf.train.Saver()
    saver.restore(sess,tf.train.latest_checkpoint('./vggvar/'))
...
def train():
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1,101):
            ...
            if epoch%2==0: save('vggvar/var.ckpt',sess)
...
def test():
    ...
    with tf.Session() as sess:
        ...
        try: restore(sess); print('Variables restored')
        except: print('Trouble Restoring Variables, terminating...')
        print('Generation csv...')
        for i in range(1,101):
            batchimages=images[100*(i-1):100*(i)]
            predictlabels=sess.run(predict_y,feed_dict={x: batchimages})
            for label in predictlabels:
                greatest=heapq.nlargest(3,label)
                index=[]
                for i in greatest: index.append(label.index(i))
                categories=[]
                for i in index:
                    if i==0: categories.append('airplane')
                    if i==1: categories.append('automobile')
                    if i==2: categories.append('bird')
                    if i==3: categories.append('cat')
                    if i==4: categories.append('deer')
                    if i==5: categories.append('dog')
                    if i==6: categories.append('frog')
                    if i==7: categories.append('horse')
                    if i==8: categories.append('ship')
                    if i==9: categories.append('truck')
                labellist.append(categories)
    with open('VGG.csv', 'wt') as csvfile:
